...
def _is_blue_pen(img: np.ndarray, x_min: int) -> bool:
    ...
    Verifica a proporção de pixels com B-R > 50 na área do gabarito.
    """
...
def _dark_fill(gray: np.ndarray, cx: int, cy: int, r: int, threshold: int = 120) -> float:
    """Fill de pixels escuros (caneta preta/lápis)."""
    inner = max(int(r * 0.55), 3)
...
def _find_marked_br(img: np.ndarray, x_min: int, h: int) -> list:
    """Detecta círculos marcados com caneta azul via B-R diff forte (>150)."""
    b    = img[:, :, 0].astype(np.int16)
    r_ch = img[:, :, 2].astype(np.int16)
    diff = np.clip(b - r_ch, 0, 255).astype(np.uint8)
        ...
        for j in range(i + 1, len(raw_cols)):
            for idx_i in range(n):
                for idx_j in range(idx_i + 1, n):
                    ...
                    if score > best_score:
                        best_score, best_result = score, [int(round(g)) for g in grid]
    return best_result


def _infer_cols_from_marked(marked_xs: list, x_min: int, w: int, n: int = 5) -> list:
...
def read_gabarito(source, num_questions: int = None) -> list:
    ...
    Parâmetros
    ----------
    source        : bytes (HTTP) ou str com caminho do arquivo.
    num_questions : número de questões; se None, detecta automaticamente.

    Retorna
    -------
    [{"question_number": int, "student_answer": str | None}, ...]
    """
    NUM_OPTIONS      = 5
    ...
    if hough is None:
        raise RuntimeError("Nenhum círculo detectado. Verifique a qualidade da imagem.")

    hough_circles, hough_rows, hough_cols = hough
    avg_r = int(np.mean([r for cx, cy, r in hough_circles])) if hough_circles else int(h * 0.022)
    ...
    if blue_pen:
        ...
        if len(blue_marked) >= 2:
            col_centers = _infer_cols_from_marked([m[0] for m in blue_marked],
                                                  x_min, w, n=NUM_OPTIONS)
    if not col_centers:
        col_centers = _regularize_cols(hough_cols, n=NUM_OPTIONS) or hough_cols

    # ── 5. Combina linhas dos marcados azuis + HoughCircles ───────────────
    all_rows = set(_cluster([m[1] for m in blue_marked], tol=int(h * 0.025))) if blue_marked else set()
    ...
    for q_num, ry in enumerate(question_rows, start=1):
        ...
        if blue_pen:
            ...
            fills = [_dark_fill(gray, cx, ry, avg_r) for cx in col_centers]
        ...
        marked = (
            best_fill >= FILL_MIN
            and (avg_o == 0 or best_fill >= avg_o * RELATIVE_FACTOR)
